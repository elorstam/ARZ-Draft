...
bool CircleRenderAdapter::supports(arz::cad::EntityType type) const noexcept { return type == arz::cad::EntityType::Circle; }
bool CircleRenderAdapter::append(const arz::cad::CadEntity& entity,
    const arz::core::Document& document, const RenderContext& context, RenderScene& scene) const {
    const auto* value = dynamic_cast<const arz::cad::CircleEntity*>(&entity);
    ...
    scene.append({value->id(), value->center(), value->radius(),
