...
bool testLineInputStateMachine() {
    ...
    const auto chained = input.acceptPoint({50.0, 60.0});
    if (!chained
        || chained->start != arz::geometry::Point2D{30.0, 40.0}
        || chained->end != arz::geometry::Point2D{50.0, 60.0}
        || input.firstPoint() != arz::geometry::Point2D{50.0, 60.0}
        || !input.canUndo() || !input.undoLastSegment()
        || input.firstPoint() != arz::geometry::Point2D{30.0, 40.0}) return false;
